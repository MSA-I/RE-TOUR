...
// QA Validation using Lovable AI
async function runQAValidation(
  ...
  if (!lovableApiKey) {
    console.log("LOVABLE_API_KEY not set, skipping QA validation");
    return { passed: true, reason: "QA skipped - API key not configured" };
  }

  console.log("Running QA validation on render output...");
    ...
    const response = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: `You are a quality assurance AI for architectural and interior design image transformations. 
...
- Small artifacts at change boundaries are acceptable`,
          },
          {
            ...
            content: [
              {
                type: "text",
                text: `CHANGE REQUEST: "${changeRequest}"\n\nPlease compare the BEFORE and AFTER images. Did the change match the request without unintended modifications?`,
              },
              {
                type: "image_url",
                image_url: { url: `data:${originalMimeType};base64,${originalImageBase64}` },
              },
              {
                type: "image_url",
                image_url: { url: `data:${outputMimeType};base64,${outputImageBase64}` },
              },
            ],
          },
        ],
        tools: [
          {
            ...
            function: {
              name: "qa_result",
              description: "Return the QA validation result",
              parameters: {
                type: "object",
                properties: {
        ...
        tool_choice: { type: "function", function: { name: "qa_result" } },
      }),
    });
...
// Generate improved prompt based on QA rejection
async function generateImprovedPrompt(
  ...
  if (!lovableApiKey) {
    console.log("LOVABLE_API_KEY not set, using original prompt");
    return previousPrompt;
  }

  console.log("Generating improved prompt based on QA rejection...");
    ...
    const response = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: `You are an expert at crafting prompts for AI image editing. 
...
6. Output ONLY the revised prompt text, nothing else`,
          ...
          {
            role: "user",
            content: `Original change request: "${originalChangeRequest}"
