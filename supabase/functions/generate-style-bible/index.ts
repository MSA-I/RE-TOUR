...
serve(async (req) => {
  ...
  try {
    ...
    if (!openAiApiKey) {
      console.error("API_OPENAI not configured");
...
REFERENCE IMAGES:`;
    ...
    // Call OpenAI API for intelligent style mixing
    console.log("Calling OpenAI API for intelligent style mixing...");
    const aiResponse = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: `You are an expert interior designer and visual analyst. Your specialty is analyzing multiple design references and BLENDING their styles into one cohesive direction. You understand that style elements (materials, colors, mood, lighting) can be extracted from any room type and applied to different spaces. Always respond with valid JSON.`
          },
          {
            role: "user",
    ...
    const responseContent = aiData.choices?.[0]?.message?.content?.trim();
