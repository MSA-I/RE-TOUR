-- Manual approval transaction for floorplan pipeline steps
-- This is used by the backend function `approve-pipeline-step-manually`.

CREATE OR REPLACE FUNCTION public.manual_approve_floorplan_pipeline_step(
  p_pipeline_id uuid,
  p_step_number integer,
  p_owner_id uuid,
  p_output_upload_id uuid DEFAULT NULL,
  p_notes jsonb DEFAULT '{}'::jsonb
)
RETURNS public.floorplan_pipelines
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Persist review decision (reviewer stored in notes JSON since schema has no reviewer column)
...
REVOKE ALL ON FUNCTION public.manual_approve_floorplan_pipeline_step(uuid, integer, uuid, uuid, jsonb) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.manual_approve_floorplan_pipeline_step(uuid, integer, uuid, uuid, jsonb) TO authenticated;