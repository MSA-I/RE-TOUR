...
export function useFloorplanPipelines(projectId: string) {
    ...
    mutationFn: async ({ 
      ...
      const { data, error } = await supabase
        ...
        .insert({
          ...
          status: `step${startFromStep}_pending`,
  ...
  const startStep = useMutation({
    mutationFn: async ({ 
      ...
      forwardDirection,
      designRefUploadIds,
      styleTitle, // Human-readable style name for Step 2
      outputCount = 1, // NEW: Number of outputs to generate (1-4 for Steps 2, 3, 4)
      isAutoRerender = false,
      autoRerenderAttempt = 0, // Track attempt number for auto-rerender loop
      step3PresetId, // Step 3: User-selected preset ID
      step3CustomPrompt // Step 3: User-provided custom prompt
    }: { 
      pipelineId: string; 
      ...
      if (data?.autoRerender === true) {
        ...
        // Wait a short moment then trigger another render
        await new Promise(r => setTimeout(r, 1500));
        
        // Recursively call to continue the auto-rerender loop
        return startStep.mutateAsync({ 
          pipelineId, 
          cameraPosition: data.camera_position,
          forwardDirection: data.forward_direction,
          designRefUploadIds, // Preserve design refs in auto-rerender
          styleTitle, // Preserve style title in auto-rerender
          isAutoRerender: true,
          autoRerenderAttempt: data.attempt // Pass the attempt counter
        });
      }
    ...
    onSuccess: (data, variables) => {
      ...
      // Only show success toast if this wasn't an auto-rerender intermediate step
      if (data?.autoRerender !== true) {
