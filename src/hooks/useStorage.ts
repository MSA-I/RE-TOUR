...
export function useStorage() {
  ...
  const getSignedViewUrl = useCallback(async (bucket: string, path: string, expiresIn = 3600): Promise<SignedUrlResult> => {
    const accessToken = await getAccessToken();
  ...
  const getSignedDownloadUrl = useCallback(async (bucket: string, path: string, filename?: string, expiresIn = 3600) => {
    const accessToken = await getAccessToken();

    const { data, error } = await invokeWithRetry<any>(
      "create-signed-download-url",
      { bucket, path, expiresIn, filename },
      accessToken,
      { timeoutMs: 20000 }
    );
